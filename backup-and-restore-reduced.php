...
$backup = new BackupRestore($argv); # ($argv, true) für debugging
$backup->execute();
exit();
...
class BackupRestore {
	...
	public $exclude_patterns 				= array('\#snapshot/**', '@eaDir/**', '*.part', '_tmp/**', '.svn/**', '.git/**');
	...
	/* Generic variables */
	public $configFile 						= '/PATH/TO/CONFIG/config-backup-and-restore.yml';
	...
	public $move_default_command			= "mv ";
													/* Need "-O --no-t --no-perms --chmod=Du+rwx" to work for non-root users on CIFS mounts.
														Use '-e "ssh -p <ssh port>"' and "localhost:/path" for --fuzzy to work.
													*/
	public $rsync_default_command 			= "rsync #SYNCMODE# -avhO --no-t --no-perms --chmod=Du+rwx --progress --delete --update --fuzzy ";
	...
	public $from_replacements 				= array(
													self::TYPE_LOCAL => array('/volume1/' => '/Backup/volume1/'),
	...
	*/
	public function parseInput($commandLine) {
		...
		$this->options = AdorableIllusion_UtilsProcess::parseCommandLine($commandLine, 'options');
		$this->arguments = AdorableIllusion_UtilsProcess::parseCommandLine($commandLine, 'arguments');
...
";			
	...
	*/
	public function autoCompleteTo($targetName, $subtarget, $to) {
		...
		if ($this->getTargetField($targetName, 'override_backup_replacements') === null) {
...
*************************************************************************************************************
 ...
 Backup predefined target(s)
 	...
 	$scriptName backup --target=photos,cloud-core,janedoe
 ...
 Backup location to different directory/bucket
	...
	$scriptName backup /volume1/foo bucketname:
	$scriptName backup /volume1/foo bucketname:/other/dir
...
*************************************************************************************************************
 ...
 Restore predefined target(s)
 	...
 	$scriptName restore --target=photos,cloud-core,janedoe
 ...
 Restore location from default target
	...
	$scriptName restore /Backup/volume1/foo	[same as previous]
	$scriptName restore bucketname:
	$scriptName restore bucketname:/volume1/somedir
 Restore location from default target elsewhere
	$scriptName restore /volume1/foo /elsewhere
	$scriptName restore /Backup/volume1/foo /elsewhere	[same as previous]
	$scriptName restore bucketname: /elsewhere
	$scriptName restore bucketname:/somedir /elsewhere
...
";
	...
	*/
	public function parseConfigFile($setConsoleTargetIfRequired = true) {
		...
		/* Run resolver multiple times to resolve nested references where @foo resolves to @bar etc. */
		for ($i = 1; $i <= 10; $i++) {
	...
	*/
	public function resolveConfigVariables($top_source, $source) {
		if (is_array($source)) {
			...
			preg_match_all('/\{@(.+)\}/iU', $source, $matches);
			if (isset($matches[1]) && count($matches[1]) >= 1) {
				foreach ($matches[1] as $match) {
					...
					$source = str_ireplace('{@' . $match . '}', $result, $source);
				} 
			}
